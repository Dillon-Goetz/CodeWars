...
function unscramble(scrambledWord) {
  const anagramMap = {}; // Use 'anagramMap' for clarity
  ...
  // Process the scrambled word:
  const sortedScrambled = scrambledWord.split('').sort().join('');

  // Look up and return anagrams:
  return anagramMap[sortedScrambled] || []; // Return the array or an empty array if not found
}
